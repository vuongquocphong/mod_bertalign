...
def continuous_numbers(start, end, step=1.0):
    ...
    if len(numbers) == 0 or numbers[-1] != end:
        numbers.append(round(end, 2))
...
skip_cost = continuous_numbers( -1.0, 0.0, 0.1 )
snt_num_penalty = continuous_numbers( 0.0, 0.5, 0.02 )
...
def run_alignment_par(src_pars, tgt_pars, golden_par, name):
    aligner = bertalign.BertalignModified(src=src_pars, tgt=tgt_pars, model=model)
...
prename = "DaiBamChinhBienLietTruyen"
